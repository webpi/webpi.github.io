 ...
 *      Sample HTML:
 *
 *          <!-- build .tab_arrow and .tab-content in here -->
 ...
 *
 *      $('.tab_arrow').scrollingTabs('refresh');
 *
 *    - scrollToActiveTab
 ...
 *
 *    $('.tab_arrow').scrollingTabs('scrollToActiveTab');
 *
 *
 *    Events
 *    -----------------------------
 ...
 *    To destroy:
 *
 *      $('.tab_arrow').scrollingTabs('destroy');
 *
...
;(function ($, window) {
  ...
  (function (p) {
      ...
      p.initElements = function (options) {
        ...
        ehd.refreshAllElementSizes(options);
      ...
      p.refreshAllElementSizes = function () {
    	...
    	var tabL;
        var tablenth = $tabLi.length; // 탭 개수
        var noLimit = stc.$tabsUl.hasClass("nolimit"); // 탭 개수 무

        if(pageW > 1200 && !noLimit){

		  tabL = 5;

	    } else if (pageW > 1200 && noLimit) {
        ...
        if (stc.scrollArrowsVisible) {
          ...
          /* 스크립트 로드시 좌측 화살표 hide 처리 */

          if(0 == stc.movableContainerLeftPos){

            $(stc.$slideRightArrow).hide();
      ...
      p.setElementReferences = function (settings) {
        ...
        stc.$tabsUl = $tabsContainer.find('.tab_arrow');
      ...
      p.setElementWidths = function (tabL) {
        var ehd = this,
            stc = ehd.stc;
        ...
        ehd.setFixedContainerWidth(tabL);
        ehd.setMovableContainerWidth(tabL);
      };
      ...
      p.setFixedContainerWidth = function (tabL) {
        ...
        var tabW = stc.fixedContainerWidth / tabL;

        stc.$fixedContainer.find(".tab_arrow > li").width(tabW);
      ...
      p.setFixedContainerWidthForVisibleScrollArrows = function () {
       ...
       /*  20-05-13 주석 stc.$fixedContainer.width(stc.fixedContainerWidth - stc.scrollArrowsCombinedWidth); */
      };

      p.setMovableContainerWidth = function (tabL) {
        var ehd = this,
            stc = ehd.stc,
            $tabLi = stc.$tabsUl.find('> li');
        ...
        stc.movableContainerWidth = (stc.fixedContainerWidth / tabL) * $tabLi.length;
  ...
  (function (p) {
    ...
    p.decrementMovableContainerLeftPos = function (minPos) {
      ...
      stc.movableContainerLeftPos -= (stc.$movableContainer.width() / stc.$tabsLiCollection.length);
      ...
      /* 우측 화살표 hide 처리 */
      if(minPos >= stc.movableContainerLeftPos){

        $(stc.$slideLeftArrow).hide();
        $(stc.$slideRightArrow).show();
    ...
    p.getMinPos = function () {
      ...
      return stc.scrollArrowsVisible ? (stc.fixedContainerWidth - stc.movableContainerWidth) : 0;
  ...
  (function (p) {
    p.initTabs = function (options, $scroller, readyCallback, attachTabContentToDomCallback) {
      ...
      function initTabsAfterTimeout() {
        ...
        // have the .tab_arrow hidden to prevent the clunky flash of
        // multi-line tabs on page refresh, so we need to make sure
        // they're visible before trying to wrap them
        $scroller.find('.tab_arrow').show();
  ...
  var tabElements = (function () {
    ...
    function getNewElScrollerElementWrappingNavTabsInstance($navTabsInstance, settings) {
      var $tabsContainer = $('<div class="scrtabs-tab-container"></div>'),
          leftArrowContent = settings.leftArrowContent || '<div class="scrtabs-tab-scroll-arrow scrtabs-tab-scroll-arrow-left"><i class="icon_left_arrow"><span class="blind">이전</span></i></div>',
          $leftArrow = $(leftArrowContent),
          rightArrowContent = settings.rightArrowContent || '<div class="scrtabs-tab-scroll-arrow scrtabs-tab-scroll-arrow-right"><i class="icon_right_arrow"><span class="blind">다음</span></i></div>',
  ...
  function buildNavTabsAndTabContentForTargetElementInstance($targetElInstance, settings, readyCallback) {
    ...
    $scroller.find('.tab_arrow > li').each(function (index) {
  ...
  function refreshDataDrivenTabs($container, options) {
    var instanceData = $container.data().scrtabs,
        ...
        $navTabs = $container.find('.scrtabs-tab-container .tab_arrow'),
  ...
  var methods = {
    ...
    init: function(options) {
      ...
      if (!settings.tabs) {
        ...
        // just wrap the selected .tab_arrow element(s) in the scroller
  ...
  function destroyPlugin() {
    ...
    if (scrtabsData.isWrapperOnly) { // we just wrapped nav-tabs markup, so restore it
      // $targetElInstance is the ul.tab_arrow
      ...
      // $targetElInstance is the container for the ul.tab_arrow we generated
      $targetElInstance
