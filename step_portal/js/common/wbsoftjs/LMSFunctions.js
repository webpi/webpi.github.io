...
function videoReSize(width, height){
	var param = "";
	var cntns_width = jsNvl(width, 1280);
	var cntns_height = jsNvl(height, 720);

	var windowH = jQuery(parent.window).height() - parent.headerH - parent.footerH;
	var mobileH = jQuery(parent.window).height();

	var height = "";
	var width = "";
	var v_height = "";
	var v_width = "";
	if(!isNull2(cntns_width) && !isNull2(cntns_height)) {
		if (cntns_width <= parent.pageW) {
			if (jQuery("body", window.parent.document).hasClass("mob")) {
				if (cntns_height <= mobileH) {
					height = cntns_height;
				} else {
					height = mobileH;
				}
			} else {
				if (cntns_height <= windowH) {
					height = cntns_height;
				} else {
					height = windowH;
				}
			...
			v_width = "";
			v_width = Math.floor(height / 9);
			v_width = Number(v_width) * 16;

			if (v_width <= cntns_width) {
				width = v_width;
			}else{
				width = cntns_width;
			}
			...
			v_height = "";
			v_height = Math.floor(parent.pageW / 16);
			v_height = Number(v_height) * 9;
