...
function Tree(tid, rootCode, rootName, basePath)
{
   this.iconNames = new Array('blank.gif', //0
       'vertical_line.gif', //1
       'closed_lastnode.gif', //2
       'closed_node.gif', //3
       'opened_lastnode.gif', //4
       'opened_node.gif', //5
       'lastnode.gif', //6
       'node.gif', //7
       'closed_default_folder.gif', //8
       'opened_default_folder.gif');//9
   /**
    * Icon Cache
    */
    ...
    */
   this.selectedBackground = "0000ff";
   /**
    * Foreground for selected node
    */
   this.selectedForeground = "#000fff";
   /**
    * Font size
    */
   this.fontSize = "12";
    ...
    * Root Node of tree
    */
   this.root = new TreeNode(1, rootCode, "*", rootName,"","L","N",-1, "0000000");
   /**
    * Preload icons
    */
   ...
   this.repaintForIE = function()
   {
      for(var i = 0; i < this.changedNodes.length; i++)
      {
         ...
         //alert(currentNode.parent.isCollapsed);
      ...
      if(node.index > 0){
         if(node.leftside.length > 0){
            for(var i = 0; i < node.leftside.length; i++)
            {
               ...
               if(node.isSelected()){
                  titClass = "this";
               } else if(node.completeyn == "Y") {
                  titClass = "complete";
               } else if(node.completeyn == "N" && !isNull2(node.opendate)){
                  titClass = "ing";
               }
               tag += " <div class='learning_dep2'>";
               tag += "   <a href='javascript:void(0)' class='tit "+titClass+"' onClick='nodes[" + node.index + "].tree.treeSelectionListener(nodes[" + node.index + "])'>";
               tag += "      <div class='tit_name'>"+node.name+"</div>";
               tag += "   </a>";
               ...
               tag += "   <a href='javascript:void(0)' id='item_idx_"+node.code+"' class='le_bookmark "+memoClass+"' onclick='jsStudyMemo(\""+node.code+"\");'>";
               tag += "         <span class='blind'>학습 메모</span>";
               tag += "         <i class='icon_bookmark'></i>";
               tag += "   </a>";

               tag += " </div>";
               renderString = renderString + tag;
            }
         }else{
            var tag = "";
            var titClass = "";
            var chasiStatusNm = "";
            //console.log(node);
            if(node.chasiStatus == "C"){
               titClass = "complete";
               chasiStatusNm = "학습완료";
            }else if(node.chasiStatus == "I"){
               titClass = "ing";
               chasiStatusNm = "학습중";
            }else if(node.chasiStatus == "R"){
               titClass = "wait";
               chasiStatusNm = "학습대기";
            }

            tag += " <div class='learningBox'>";
            tag += "    <div class='info'>";
            tag += "       <div class='infoL'>";
            tag += "          <span class='le_class'>"+node.chasiid+"차시</span>";
            tag += "          <span class='le_state "+titClass+"'>"+chasiStatusNm+"</span>";
            tag += "       </div>";
            if(!isNull2(node.noteUrl)){
               tag += "       <div class='infoR'>";
               tag += "          <a href='javascript:void(0);' class='download' onclick='jsFileUrl(\""+node.noteUrl+"\")'>";
               tag += "             <i class='icon_download'></i>";
               tag += "             <span class='blind'>강의 자료 다운로드</span>";
               tag += "          </a>";
               tag += "       </div>";
            }
            tag += "    </div>";
            tag += "    <a href='javascript:void(0);' class='tit' onClick='nodes[" + node.index + "].tree.toggleNode(nodes[" + node.index + "])'>";
            tag +=         node.name;
            tag += "    </a>";
            tag += " </div>";
            renderString = renderString + tag;
   ...
   {
      var renderString = "";
      //console.log("node.leftside.length : " + node.leftside.length);
      for(var i = 0; i < node.leftside.length; i++)
      {
         ...
         //console.log("for node.leftside.length renderString : " + renderString);
      }
      if(node != this.root)
      {
         if(node.isLeaf())
         {
            ...
            //console.log("node.isLeaf() true renderString : " + renderString);
         }
         else
         {
            var nodeIcon = null;
            ...
            //console.log("node.isLeaf() false renderString : " + renderString);
         }
      }
      ...
      if(this.openedNode == node)
      {
         ...
         //console.log("this.openedNode true renderString : " + renderString);
      }
      else
      {
         ...
         //console.log("this.openedNode false renderString : " + renderString);
      }
      ...
      renderString = renderString + "<SPAN style='color:" + fg + ";background-color:" + bg + ";font-size:" + this.fontSize + ";font-family:" + this.fontName + ";cursor:default'" + " onClick='nodes[" + node.index + "].tree.treeSelectionListener(nodes[" + node.index + "])'><font color='#000000'>" + node.name + "</font></SPAN>";

      //console.log("isSelectedNode true renderString : " + renderString);
      return renderString;
   }
   /**
...
function TreeNode(chasiid, code, upcode, name, filename, type, completeyn, arrnum, contsid, chasiStatus, noteUrl, memoRegYn, opendate)
{
   /**
    * node index
    */
   ...
   this.clone = function()
   {
      var newNode = new TreeNode(this.chasiid, this.code, this.upcode, this.name, this.filename, this.type, this.completeyn, this.arrnum, this.contsid, this.chasiStatus, this.noteUrl, this.memoRegYn, this.opendate);
...
function setLookAndFeel(dir){
   tree.iconDir="images/treeicon/"+dir;
   tree.loadIcons();
   tree.repaintAll();
}


function createNode(chasiid, cateid, upcateid, catename, filename, type, completeyn, arrnum, contsid, chasiStatus, noteUrl, memoRegYn, opendate){
   var temp = new TreeNode(chasiid, cateid, upcateid, catename, type, completeyn, arrnum, contsid, chasiStatus, noteUrl, memoRegYn, opendate);
   temp.put("link", cateid);
   try{
      tree.selectedNode[0].add(temp);
      tree.repaint();
   }catch(e){
      //다국어원본 : 생성할 노드의 부모노드를 선택하셔야 합니다.
      alert(MESSAGE["parentNode"][jslang]); //다국어phs
   }
}
...
function deleteNode(){
   try{
      if(tree.selectedNode[0]==tree.root){
         //다국어원본 : 루트 노드는 삭제할 수 없습니다.
         alert(MESSAGE["rootNodeDelete"][jslang]); //다국어phs
         return;
      }
      ...
      //다국어원본 : 삭제할 노드를 선택하셔야 합니다.
      alert(MESSAGE["deleteNodeSelect"][jslang]); //다국어phs
   }
}

function find(top_node, findTxt)
{
   var child_node = null;
   var find_node = null;

   if(top_node != null){
      if(top_node.children != null){
         for(var i =0; i < top_node.children.length; i++){
            child_node = top_node.children[i];

            //alert("i =>"+i+",child =>"+child_node.code);
            if(child_node != null)
               if(child_node.code ==  findTxt) {
                  //alert("findTxt =>"+findTxt+",child =>"+child_node.code);
                  return child_node;
               }
               else {
                  var find_node = find(child_node, findTxt);

                  if(find_node != null) return find_node;
               }
...
function find_toggleNode(cateid){
   var findNode = find(tree.root, cateid);
   expandParentNode(findNode.parent);
   //tree.treeSelectionListener(findNode);
...
function expandParentNode(node){
   ...
   if(node != null && node.isCollapsed == 1) tree.toggleNode(node);
}

function find_createNode(chasiid, cateid, upcateid, catenamen, filename, type, completeyn, arrnum, contsid, chasiStatus, noteUrl, memoRegYn, opendate){
   var findChild = find(tree.root, upcateid);

   if(findChild == undefined) findChild = tree.root;
   node = new TreeNode(chasiid, cateid, upcateid, catename, filename, type, completeyn, arrnum, contsid, chasiStatus, noteUrl, memoRegYn, opendate);
   node.put("link",cateid);

   findChild.add(node);
}

function find_refreshNode(cateid, upcateid, catename){
   var findNode = find(tree.root, cateid);

   if(findNode == null) return;
   findNode.name = catename;
   //tree.treeSelectionListener(findNode);
   if(findNode.isCollapsed == 1) tree.toggleNode(findNode);
}

function find_deleteNode(cateid){
   try{
      var preNode = tree.selectedNode[0].parent;
      var findNode = find(tree.root, cateid);
      findNode.removeFromParent();
      tree.treeSelectionListener(preNode);
      tree.repaint();
   }catch(e){
      //다국어원본 : 삭제할 노드를 선택하셔야 합니다.
      alert(MESSAGE["deleteNodeSelect"][jslang]); //다국어phs
