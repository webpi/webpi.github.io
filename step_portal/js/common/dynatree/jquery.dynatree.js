...
//          noop(); // keep JSLint happy
			...
			if(data.key == "0"){
				...
				res += "<span class='dynatree-icon folder_Top'></span>";
			}
		}
		if (data.menuicon != "") {
			if(data.menuicon == '01'){	
				//res +=  cache.tagmenuicon;	//카테고리
				res +=  "<span class='dynatree-icon folder_m'></span>";	//카테고리
			}else if(data.menuicon == '02'){	
				//res +=  cache.tagmenuicon;	//메뉴
				res +=  "<span class='dynatree-icon folder_menu'></span>";
			}else if(data.menuicon == '03'){
				//res +=  cache.tagmenuicon;	//메뉴상세
				res +=  "<span class='dynatree-icon folder_Tab'></span>";
			}else if(data.menuicon == '04'){
				//res +=  cache.tagmenuicon;	//기능
				res +=  "<span class='dynatree-icon folder_function'></span>";
			}else if(data.menuicon == '05'){
				//res +=  cache.tagmenuicon;	//팝업
				res +=  "<span class='dynatree-icon folder_Pop'></span>";
			}else if(data.menuicon == '06'){
				//res +=  cache.tagmenuicon;	//웹모듈
				res +=  "<span class='dynatree-icon folder_Module'></span>";
			}else if(data.menuicon == '07'){
				//res +=  cache.tagmenuicon;	//직접등록
				res +=  "<span>(D)</span>";
			}
		} else {
			res += cache.tagNodeIcon;
...
//              alert("HIT "+ cn.className);
				if( cn.className==cns.title ){
					...
					return "icon";
				}else if (cn.className==cns.menuicon){
					return "menuicon";
				}
			}
		}
	...
	getEventTargetType: function(event) {
		...
		if( tcn.indexOf(cns.title) >= 0 ){
			...
			return "icon";
		}else if(tcn.indexOf(cns.menuicon) >= 0){
			return "menuicon";
...
//      _log("warn", "jQuery.support.boxModel " + jQuery.support.boxModel);
		...
		this.cache = {
			...
			tagmenuicon: "<span class='" + opts.classNames.menuicon + "'></span>",
			lastentry: undefined
		};
