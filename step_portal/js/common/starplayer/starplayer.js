...
var isAtLeastIE11 = !!(navigator.userAgent.match(/Trident/) && !navigator.userAgent
		.match(/MSIE/));
var isIE11 = !!(navigator.userAgent.match(/Trident/) && navigator.userAgent
		.match(/rv 11/));
...
function installStarPlayer() {
	var a = "<object classid='CLSID:99277D5A-52B3-4B2E-AC38-B0065575FC55' width='0' height='0' codebase='"
			+ STARPLAYER_URL
			+ "#version="
			+ STARPLAYER_VERSION
			+ "' ></object>";
...
function StarPlayer(f, d, i) {
	...
	var l = false;
	var b = null;
	var a = window.navigator.platform.toLowerCase() == "win64"
	...
	function o() {
		...
		function t() {
			k(f.videoContainer).innerHTML = "<table width='100%' height='100%' style='color:white;font-size:14px'><tr><td align='center' valign='middle'><p>스타플레이어를 구동하려면 <a style='color:#ff0000' href='"
					+ STARPLAYER_SETUP_URL
					+ "'>설치 프로그램을 다운로드</a>한 후 설치하여 주십시오.</p></td></tr></table>";
		...
		function D() {
			k(f.videoContainer).innerHTML = "<table width='100%' height='100%' style='color:white;font-size:14px'><tr><td align='center' valign='middle'><p>스타플레이어를 구동하려면 'StarPlayer' ActiveX 컨트롤을 설치하여 주십시오.</p><p>설치에 문제가 있으면 <a style='color:#ff0000' href='"
					+ STARPLAYER_SETUP_URL
					+ "'>설치 프로그램을 다운로드</a>한 후 설치하여 주십시오.</p></td></tr></table>";
			if (isAtLeastIE11) {
				setInterval(function() {
		...
		function x(G) {
			...
			I = I != "" ? I : "100%";
			var H = "<object style='position:absolute;left:-9999999px;' id='"
					+ c
					+ "' classid='CLSID:99277D5A-52B3-4B2E-AC38-B0065575FC55' width='"
					+ J + "' height='" + I + " codebase='" + STARPLAYER_URL
					+ "#version=" + STARPLAYER_VERSION
					+ "' ><param name='config' value='" + STARPLAYER_CONFIG_URL
					+ "' /><param name='controller' value='" + n
					+ "' /><param name='user_id' value='" + f.userId
					+ "' /><param name='time_limit' value='" + d.previewTime
					+ "' /><param name='auto_play' value='" + d.autoPlay
					+ "' /><param name='video_armode' value='" + v
					+ "' /><param name='cpcode' value='" + f.cpcode
					+ "' /><param name='controller_container_hwnd' value='" + G
					+ "' /><param name='controller_params' value='"
					+ f.controllerParams
					+ "' /><param name='enable_block_messenger' value='"
					+ f.blockMessenger
					+ "' /><param name='enable_block_virtual_machine' value='"
					+ f.blockVirtualMachine
					+ "' /><param name='enable_dual_monitor' value='"
					+ f.dualMonitor
					+ "' /><param name='closed_caption_size' value='"
					+ f.captionSize
					+ "' /><param name='watermark_text' value='"
					+ f.watermarkText
					+ "' /><param name='watermark_text_color' value='"
					+ f.watermarkTextColor
					+ "' /><param name='watermark_text_size' value='"
					+ f.watermarkTextSize
					+ "' /><param name='watermark_horz_align' value='"
					+ f.watermarkHorzAlign
					+ "' /><param name='watermark_vert_align' value='"
					+ f.watermarkVertAlign
					+ "' /><param name='watermark_interval' value='"
					+ f.watermarkInterval
					+ "' /><param name='watermark_show_interval' value='"
					+ f.watermarkShowInterval
					+ "' /><param name='auto_progressive_download' value='"
					+ f.auto_progressive_download
					+ "' /><param name='marker' value='" + d.marker
					+ "' /><param name='controller_mode' value='"
					+ f.controllerMode
					+ "' /><param name='custom_log_url' value='" + d.logUrl
					+ "' /><param name='custom_log_extra' value='"
					+ d.logExtraData + "' /></object>";
		...
		function A(G) {
			if (E()) {
				k(f.videoContainer).innerHTML += "<object style='visibility:hidden' id='"
						+ c
						+ "' width='100%' height='100%' type='application/x-starplayer' codebase='"
						+ STARPLAYER_SETUP_URL
						+ "#version="
						+ STARPLAYER_VERSION
						+ "' ><param name='config' value='"
						+ STARPLAYER_CONFIG_URL
						+ "' /><param name='controller' value='"
						+ n
						+ "' /><param name='user_id' value='"
						+ f.userId
						+ "' /><param name='time_limit' value='"
						+ d.previewTime
						+ "' /><param name='auto_play' value='"
						+ d.autoPlay
						+ "' /><param name='video_armode' value='"
						+ v
						+ "' /><param name='cpcode' value='"
						+ f.cpcode
						+ "' /><param name='controller_container_hwnd' value='"
						+ G
						+ "' /><param name='controller_params' value='"
						+ f.controllerParams
						+ "' /><param name='enable_block_messenger' value='"
						...
						+ f.watermarkShowInterval
						+ "' /><param name='auto_progressive_download' value='"
						+ f.progressive_download
						+ "' /><param name='marker' value='"
						+ d.marker
						+ "' /><param name='controller_mode' value='"
						+ f.controllerMode
						+ "' /><param name='custom_log_url' value='"
						+ d.logUrl
						+ "' /><param name='custom_log_extra' value='"
						+ d.logExtraData + "' /></object>";
				if (f.visible != false) {
		...
		function B() {
			...
			H = H != "" ? H : "100%";
			k(f.controllerContainer).innerHTML = "<object style='position:absolute;left:-9999999px;' id='"
					+ h
					+ "' classid='CLSID:7A63FEE6-E174-4FBC-A064-875DB95594A6' width='"
					+ I
					+ "' height='"
					+ H
					+ "' codebase='"
					+ STARPLAYER_URL
					+ "#version=" + STARPLAYER_VERSION + "' ></object>";
			var G = k(h);
			if (isAtLeastIE11) {
				if (G.object) {
		...
		function u() {
			if (E()) {
				k(f.controllerContainer).innerHTML = "<object id='"
						+ h
						+ "' type='application/x-starplayer' width='100%' height='100%'><param name='uimode' value='true' /></object>";
				r()
			} else {
				t()
			}
	...
	this.getAlphaBrain = function() {
		function q() {
			...
			this.start = function(u) {
				m
						.control_alpha_brain("AB_Start", u ? u : [ s, r, t ]
								.join("/"))
			};
			this.stop = function() {
