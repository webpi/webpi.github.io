...
function displayWorkProgress(type) {
	...
	try {
		if(whole_tmplcd == "9"){
			boxContent = "";
			boxContent += '<div class="loading">';
			boxContent += '<div class="loading-box">';
			boxContent += '<svg class="loading-spin" viewBox="25 25 50 50">';
			boxContent += '<circle class="loading-path" cx="50" cy="50" r="20" fill="none" stroke-width="2" stroke-miterlimit="10"></circle>';
			boxContent += '</svg>';
			boxContent += '<span class="loading-txt">loading</span>';
			boxContent += '</div>';
			boxContent += '</div>';
		}
	} catch (e) {
