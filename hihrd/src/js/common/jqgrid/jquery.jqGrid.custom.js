...
$.fn.jqGrid = function( pin ) {
	...
	return this.each( function() {
		...
		formatCol = function (pos, rowInd, tv, rawObject, rowId, rdata){
			...
			if(cm.editable===true && ts.p.cellEditForm === true) { result += "color:blue;"; } //custom
			if(rowInd===0) {
		...
		addXmlData = function (xml,t, rcnt, more, adjust) {
			...
			if(ts.p.cellEditForm){//custom
				//$(ts).jqGrid("initGrid",xml);			
			}
			ts.p.reccount = 0;
			if($.isXMLDoc(xml)) {
		...
		addLocalData = function() {
			...
			total = ts.p.records,//custom
			//total = queryResults.length,
			page = parseInt(ts.p.page,10),
			tmpPage = 1;	//custom
			var totalpages = Math.ceil(total / recordsperpage),
			retresult = {};				
			...
			queryResults = queryResults.slice( (tmpPage-1)*recordsperpage , tmpPage*recordsperpage );
			query = null;
			cmtypes = null;
		...
		setPager = function (pgid, tp){
			...
			if(ts.p.rowList.length >0){
				...
				for(i=0;i<ts.p.rowList.length;i++){
					str +="<option role=\"option\" value=\""+ts.p.rowList[i]+"\""+((Number(ts.p.rowNum) == Number(ts.p.rowList[i]))?" selected=\"selected\"":"")+">"+ts.p.rowList[i]+"</option>";
				}
				str +="</select></td>";
			}
			if(dir==="rtl") { pgl += str; }
			...
			$("#first"+$.jgrid.jqID(tp)+", #prev"+$.jgrid.jqID(tp)+", #next"+$.jgrid.jqID(tp)+", #last"+$.jgrid.jqID(tp)).click( function() {
				...
				if(selclick) {//custom		
					if(ts.p.ajaxCallFunc ==true){//custom
						ts.p.datatype = 'json';
					}
					populate();					
				}
		...
		sortData = function (index, idxcol,reload,sor, obj){
			...
			if(!reload) {
				...
				if(ts.p.ajaxCallFunc ==false){//custom
					ts.p.page = 1;
				}
			}
			if(ts.p.multiSort) {
			...
			if(cSel === true) {
				...
				if(ts.p.cellEdit === true) {
					if(ts.p.multiselect && scb){
						...
						ri = ptr[0].rowIndex;
						if(ts.p.cellEditForm === true){
							$(ts).jqGrid("setSelection", ptr[0].id ,true,e);//custom수정
						}else{
		...
		.dblclick(function(e) {
			...
			if(ts.p.cellEditForm === true){
				try {$(ts).jqGrid("editCell",ri,ci,true);} catch (_) {}//custom수정
			...
			if(ts.p.cellEditForm === true){
				var cellInfo = $("td:eq("+ci+")",this.rows[ri]);
				var edit = $(".edit-cell > *");
				if($("input.hasDatepicker",cellInfo).length <= 0) {
					edit.blur(function () {
						$(ts).jqGrid('cellCompare',$(ptr).attr("id"),ri,ci);
					});
				}
			}
		})
...
$.jgrid.extend({
	...
	cellCompare : function(rowid,iRow, celid){		
		return this.each(function(){
		var initGrid = this.p.initGrid;
		var submitGrid = this.p.submitGrid; 
		
    	var colModel = $(this).jqGrid('getGridParam', 'colModel'); // 컬럼명 배열형태
    	var curCell = colModel[celid]['name'];
    	var curGrid = $(this).jqGrid("getRowData",rowid);
    	var colValue = "";
    	if(colModel[celid].edittype === 'password'){    		
    		colValue = $("#"+$(curGrid[curCell]).attr("id")).val();
    	}else{
    		$(this).saveCell(iRow,celid);
    		colValue = $(this).getCell(rowid,celid);
    	} 	
    	if(colValue == "&nbsp;" || colValue == "&#160;")colValue="" ;
    	
    	if(initGrid[rowid-1] !=null && initGrid[rowid-1] != undefined){
        	if(colModel[celid]['editable'] && initGrid[rowid-1][curCell] != colValue){
        		//수정된 값 존재
        		var upArr = this.p.updateArr;
        		var flag = true;
        		...
        		if(flag){
        			this.p.updateArr.push(rowid);
        		}
        		$(this).jqGrid('setCell', rowid,curCell,'',{ fontWeight:'bold' });	
        	}else{
        		//수정되지 않음
        		var upArr = this.p.updateArr;
        		var flag = true;
        		...
        		$(this).jqGrid('setCell', rowid,curCell,'',{fontWeight:'' });
        	}
        	
    	}else{
    		if( colValue !== "" && colModel[celid]['editable']){
    			$(this).jqGrid('setCell', rowid,curCell,'',{ fontWeight:'bold' });
    		}else{
    			$(this).jqGrid('setCell', rowid,curCell,'',{ fontWeight:'' });
    		}
    	...
    	for(var i=0;i<submitGrid.length;i++){
    		if(submitGrid[i]["rowId"] == rowid){
    			submitGrid[i][curCell] = colValue;
    		}
    	}
    	
    	this.p.submitGrid = submitGrid;
		$(this).resetSelection();
		//$(this).unSelectCell();
	...
	initGrid : function(grid){
		return this.each(function(){
			...
			this.p.initGrid = inGrid;
			this.p.submitGrid = JSON.parse(JSON.stringify(inGrid));;
			this.p.insertArr = [];
			this.p.updateArr = [];
			this.p.deleteArr = [];
		});		
	...
	insertRow : function(pos){
		...
		if(selrow == null && (pos == 'after' || pos == 'before')){
			alert("추가할 행을 지정해야 합니다.");
			return;
		}
		
		//$(this).unSelectCell();
		
		var rowid = this.p.addRowid +1;
		var initdata= {id:rowid};
		this.p.insertArr.push(rowid);
		if((pos == 'after' || pos == 'before')){
			if(this.p.addSelectid != 0 && selrow ==""){
				selrow = this.p.addSelectid;
			}
			
			$(this).jqGrid('addRowData',rowid,initdata,pos,selrow);
			 this.p.addSelectid = selrow;
		}else{
			$(this).jqGrid('addRowData',rowid,initdata,pos);
		}
		
			this.p.addRowid = rowid;
			var copyData = {};
			var colModel = $(this).jqGrid('getGridParam', 'colModel');
			
			//var copyData2= this.p.submitGrid[0];
			for(var i=0;i<colModel.length;i++){
				copyData[colModel[i]["name"]] = $(this).getCell(rowid,i);;
			}
			copyData["rowId"]=rowid;
			this.p.submitGrid.push(copyData); 
			
		});
	},
	deleteRow : function(del){
		return this.each(function(){
		var selrows = null;
		if(del){
			selrows = del;
		}else{
			selrows = $(this).jqGrid('getGridParam', "selrow" );
			if(selrows == "" || selrows == undefined){		
				return;
			}
		}
		
		//$(this).resetSelection();
		var inArr = this.p.insertArr; 
		for(var i=0;i<inArr.length;i++){
			if(inArr[i] == selrows){
				this.p.insertArr.splice(i,1);
			}
		}
		var upArr = this.p.updateArr;
		var flag = true;
		for(var i=0;i<upArr.length;i++){
			if(upArr[i] == selrows){
				this.p.updateArr.splice(i,1);
    			this.p.deleteArr.push(selrows);
    			flag=false;
			}
		} 
		if(flag){
	...
	getInsertData : function(){
		...
		for(var i=0;i<inArr.length;i++){
			var submitGrid = this.p.submitGrid;
			for(var j=0;j<submitGrid.length;j++){
	    		if(submitGrid[j]["rowId"] == inArr[i]){
	    			insertData.push(submitGrid[j]);
	    		}
	    	}			
		}
   ...
   getUpdateDataCustom : function(arr){
        var updateData = [];    
        this.each(function(){
        ...
        for(var i=0;i<upArr.length;i++){
            var submitGrid = this.p.submitGrid;
            for(var j=0;j<submitGrid.length;j++){
                if(submitGrid[j]["rowId"] == upArr[i]["rowId"]){
                    updateData.push(submitGrid[j]);
                }
            }           
        }
	...
	getDeleteData : function(){		
		...
		var delArr = this.p.deleteArr;
		delArr.sort();
		for(var i=0;i<delArr.length;i++){
			var submitGrid = this.p.submitGrid;
			for(var j=0;j<submitGrid.length;j++){
	    		if(submitGrid[j]["rowId"] == delArr[i]){
	    			deleteData.push(submitGrid[j]);
	    		}
	    	}			
		}
	...
	getEditData : function(){
		...
		$(this).resetSelection();
		editData = {
			indata : $(this).getInsertData()
			,updata : $(this).getUpdateData()
			,deldata : $(this).getDeleteData()
		}
		
		});
		return editData;
	},
   getEditDataCustom : function(arr){
        var editData
        this.each(function(){
        $(this).resetSelection();
        editData = {
            indata : $(this).getInsertData()
            ,updata : $(this).getUpdateDataCustom(arr)
            ,deldata : $(this).getDeleteData()
        }
...
$.jgrid.extend({
		...
		return this.each(function (){
			...
			if (cm.editable===true && ed===true && !cc.hasClass("not-editable-cell")) {
				...
				if(cm.formatter === 'date'){
					$(elc).datepicker({
						...
						beforeShow: function() {
							...
							$('#ui-datepicker-div').removeClass('hide-calendar');			//날짜까지 출력
							//} else {
								//$('#ui-datepicker-div').addClass('hide-calendar');			//달력만 출력
							//}
							$(".ui-datepicker-current").hide();										//달력 하단 Today 버튼 미출력
			  			},
						onSelect: function (dateText, inst) {
							//출력 유형이 날짜만(D)일 경우는 바로 셀에 저장하지만, 시간(T)까지일 경우는 날짜 뒤에 현재 시간을 붙인 후 저장한다.
							//if(datetype == "D") {
							if($t.p.initGrid != null){								
								$($t).cellCompare($t.rows[iRow].id,iRow, iCol);
							}else{								
							...
							/*	var changeTxt = "";
				        		...
				        		var nowdate = new Date(); 											//현재 시간을 가져온다.
								var getHour = nowdate.getHours();
								var getMin = nowdate.getMinutes();
								var getSec = nowdate.getSeconds();
								
								//분, 초가 10보다 작을 경우 앞에 0을 붙인다. (ex : 1분 4초 -> 01분 04초)
								if(getMin < 10) {
									getMin = "0" + getMin;
								}
								if(getSec < 10) {
								...
								if(datefmt == "HMS") {
									changeTxt = dateText + " " + getHour + ":" + getMin + ":" + getSec;
								} else if(datefmt == "HM") {
									changeTxt = dateText + " " + getHour + ":" + getMin;
								} else {
									changeTxt = "";
								}
								
								if(changeTxt != null && changeTxt != "") {
									jQuery("#dateList").changeTextSaveCell(saverow, savecol, changeTxt);
								}
							}	*/
						},
				...
				$("input, select, textarea",cc).bind("keydown",function(e) {
					...
					if (e.keyCode === 13) {
						if($t.p.cellEditForm){
							$($t).jqGrid("cellCompare",$t.rows[iRow].id,iRow,iCol); //custom
						}
					...
					if (e.keyCode === 9)  {
						if(!$t.grid.hDiv.loading ) {
							if($t.p.cellEditForm){
								$($t).jqGrid("cellCompare",$t.rows[iRow].id,iRow,iCol); //
							}
				...
				$("input, select, textarea",cc).bind("blur",function(e) {
					if($("input.hasDatepicker",cc).length <= 0) {
						if($t.p.cellEditForm){
							$($t).jqGrid("cellCompare",$t.rows[iRow].id,iRow,iCol); //custom
						}
	...
	changeTextSaveCell : function (iRow, iCol, changeValue){
		return this.each(function(){
			var $t= this, fr;
			if (!$t.grid || $t.p.cellEdit !== true) {return;}
			if ( $t.p.savedRow.length >= 1) {fr = 0;} else {fr=null;} 
			if(fr !== null) {
				...
				switch (cm.edittype) {
					...
					case 'custom' :
						try {
							alert($.isFunction(cm.editoptions.custom_value));
							if(cm.editoptions && $.isFunction(cm.editoptions.custom_value)) {
								v = cm.editoptions.custom_value.call($t, $(".customelement",cc),'get');
								if (v===undefined) { throw "e2";} else { v2=v; }
							} else { throw "e1"; }
...
$.fn.jqgridRowspan = function(colIndexs) {
	 ...
	 var model = [];
	 // 각 column의 ID를 수집.
	 jQuery.each(this.getGridParam("colModel"), function(idx, value) {
	 ...
	 jQuery('tbody tr', this).each(function(row, rowObject) {
		 var tmpIDx = 0;
		 jQuery('td', this).each(function(col, colObject) {
			 if (col == colIndexs && row > 0) {// 0번째 row는 숨겨진 row다 이것 때문에 width가 깨지는 현상 발생
				 if (rowspanData[row - 1]) tmpIDx = rowspanData[row - 1];
				 else tmpIDx = 0;
				 
				 if (tmpIDx > 0) {
					 jQuery(colObject).attr("rowspan", tmpIDx);
				 } else {
					 jQuery(colObject).hide();
				 }
			 }
		 });
	 });
...
//jqGird 셀병합(다른 셀)
$.fn.jqgridCellRowspan = function(colIndexs, mergeCol) {
...
//jqGird 그룹 셀병합(다른 셀) type:1 코드명만병합, 2코드명과 코드값 병합
$.fn.jqgridGroupCellRowspan = function(arrData, type) {
	
	// alert(arrData.length);
	
	var model = [];
	var jqGrids = this;
	var prerowspanData = {};	// 전환점 1 비전환 0
  ...
  jQuery.each(arrData, function(i) {
		var array = arrData[i].split(",");
	    if(array[0] == "") {
	    } else {
	    	colIndexs = array[0];
	        mergeCell = array[1];
	    ...
	    var data = jqGrids.getCol(model[colIndexs]);
	    ...
	    jQuery.each(data, function(idx, value) {
	    	if( i == 0){		// 첫번째 그룹
		    	if (current != value) {
		    		currentIDX = idx;
		    		rowspanData[currentIDX] = 1;
		    		prerowspanData[precnt] = 1;	// tmp그룹저장
				} else {
					rowspanData[currentIDX]++;
					prerowspanData[precnt] = 0;	// tmp그룹저장
				}
	    	}else{	// 한개 이상의 그룹일경우
	    		if (current != value || prerowspanData[precnt] != 0) {
		    		currentIDX = idx;
		    		rowspanData[currentIDX] = 1;
		    		prerowspanData[precnt] = 1;	// tmp그룹저장
				} else {
	    ...
	    jQuery('tbody tr', jqGrids).each(function(row, rowObject) {
			 var tmpIDx = 0;
			 ...
			 jQuery('td', this).each(function(col, colObject) {
				 ...
				 if(type == "2"){	
					 if ((col == colIndexs || col == mergeCell) && row > 0){
						 if (tmpIDx > 0) {
							 jQuery(colObject).attr("rowspan", tmpIDx);
						 } else {
							 jQuery(colObject).hide();
