...
function Tree(tid, rootCode, rootName)
{
    ...
    */
   this.iconDir = "/images/common/contentstreeicon";
   /**
   ...
   this.leftSideForIE = function(node)
   {
	  ...
	  if(nodeIcon == this.OPENED_NODE_ICON || nodeIcon == this.OPENED_LASTNODE_ICON)
      {
		 ...
		 if(node.isLeaf()) {
			...
			if(node.isSelected())
				folderIcon.src = this.iconDir + "/itemsel.gif";
            else if(node.completeyn == "Y")
            	folderIcon.src = this.iconDir + "/itemcheck.gif";
			else
				folderIcon.src = this.iconDir + "/item.gif";
         } 
      ...
      renderString = renderString + "<SPAN title='" + node.fullname + "' style='cursor:pointer;color:" + fg + ";background-color:" + bg + ";font-size:" + this.fontSize + ";font-family:" + this.fontName + ";cursor:hand'" + " onClick='nodes[" + node.index + "].tree.treeSelectionListener(nodes[" + node.index + "])'><font color='white'>" + node.name + "</a></SPAN>";
      return renderString;
   }
   /**
    * generate leftside
   ...
   this.leftSideForNE = function(node)
   {
      ...
      renderString = renderString + "<SPAN title='" + node.fullname + "' style='cursor:pointer;color:" + fg + ";background-color:" + bg + ";font-size:" + this.fontSize + ";font-family:" + this.fontName + ";cursor:default'" + " onClick='nodes[" + node.index + "].tree.treeSelectionListener(nodes[" + node.index + "])'><font color='#000000'>" + node.name + "</font></SPAN>";
      return renderString;
   }
   /**
...
function TreeNode(chasiid, code, upcode, name, filename, type, completeyn, arrnum, contsid, fullname)
{
   /**
    * node index
    */
   ...
   **/
   ...
   this.clone = function()
   {
      var newNode = new TreeNode(this.chasiid, this.code, this.upcode, this.name, this.filename, this.type, this.completeyn, this.arrnum, this.contsid);
...
function FormUtil()
{
   this.validate = function(form)
   {
        ...
        function createNode(chasiid, cateid, upcateid, catename, filename, type, completeyn, arrnum, contsid){
            var temp = new TreeNode(chasiid, cateid, upcateid, catename, type, completeyn, arrnum, contsid);
  ...
  function find_createNode(chasiid, cateid, upcateid, catenamen, filename, type, completeyn, arrnum, contsid){
	   var findChild = find(tree.root, upcateid);

	   if(findChild == undefined) findChild = tree.root;
	   node = new TreeNode(chasiid, cateid, upcateid, catename, filename, type, completeyn, arrnum, contsid);
